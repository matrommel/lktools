...
Public Class LKTool
    ...
    Public Shared GraphAsPicture As Boolean 'defines if Button "Kariert als Bild einfügen" is checked.
    ...
    Public Sub LoadXML()
        ...
        If xDoc.load(LKTool.myFile) Then
            ...
            GraphAsPicture = point.selectSingleNode("//GraphAsPicture").text
    ...
    Private Sub btnGraphTemplate_Click(sender As Object, e As RibbonControlEventArgs) Handles btnGraphTemplate.Click
        ...
        'Enter as image if GraphAsPicture is true, else create a table 
        If GraphAsPicture = True Then
            If isDocumentExtensionDocx() = True Then
                ...
                If CheckMSWordCompatiblityMode(document) = True Then
                    ...
                    oShp.Title = "KarierteVorlage"
                    oShp.Fill.PresetTextured(MsoPresetTexture.msoTextureCanvas)
                    oShp.Line.Visible = False
                    With oShp.Fill
                        .TextureOffsetX = 0
                        .TextureOffsetY = 0
                        .TextureHorizontalScale = 62.5
        ...
        If check = True Then
            ...
            With cursor.Tables(1) '.Borders(Word.WdBorderType.wdBorderBottom).LineWidth = WdLineWidth.wdLineWidth050pt
                ...
                With .Borders(Word.WdBorderType.wdBorderHorizontal)
                    .LineStyle = LKTool.RuledStyle
                    'Fixed LKTools Ticket #6
                    If RuledRowsTemp > 1 Then
                        .LineWidth = LKTool.RuledStyleWidth
                    End If
                    .Color = LKTool.ObjectTextColor
                End With
   ...
   ByVal strSearch As String, ByVal strReplace As String)
    ...
    Private Sub savePDFdocument(docFileNameWith As String, docFileNameWithout As String, Hidden As Boolean,
        ...
        If Hidden = True Then 'IF solution should be hidden
            ...
            If LKTool.chkPDFRevisionTemp = False And DocRangeNoSolution = 0 And document.Bookmarks.Exists("RevisionTable") = True Then
                ...
                'Fix: if mirrored Pages are actiated, an empty page is shown at the beginning, therefore it should start with page 3
                If cursor.PageSetup.MirrorMargins = True Then
                    StartNumNoSolution = 3
                Else
    ...
    Private Sub btnRevision_Click(sender As Object, e As RibbonControlEventArgs) Handles btnRevisionPage.Click
        ...
        If isDocumentExtensionDocx() = True Then
            ...
            Else
                ...
                'Define the default page layout settings to avoid two printed sizes
                'Todo Bugfix mit falscher Dokumentenseite
                cursor.PageSetup.OddAndEvenPagesHeaderFooter = False
    ...
    'if this shape is found on a page, the page should be ignored in the students document export
    Private Sub btnPrintIgnorePage_Click(sender As Object, e As RibbonControlEventArgs) Handles btnPrintIgnorePage.Click
        'Add a red textbox that can be hidden afterwards.
        Dim document As Word.Document
        Dim cursor As Word.Selection
        Dim oShp As Word.Shape 'check if shape already exists.
        ...
        'Create a red rectangle
        oShp = document.Shapes.AddTextbox(MsoTextOrientation.msoTextOrientationHorizontal, leftCursor, topCursor, 100, 30)
        oShp.Select() 'Select the created textbox
        ...
        With oShp.TextFrame.TextRange
            .Text = "Ignorie Seite im Ausdruck"
            With .Font
                .Size = 11
                .Name = "Times New Roman"
                .Bold = False
                .TextColor.RGB = ObjectTextColor
            End With
            oShp.Name = "IgnorePageInPrint"
        End With
    End Sub

    ' Todo: Funktion nutzen
    Public Function GetPrintablePages() As List(Of Integer)
        Dim app As New Word.Application
        Dim doc As Word.Document
        Dim printablePages As New List(Of Integer)
        Dim shapes As Word.Shapes
        Dim range As Word.Range

        doc = Globals.ThisAddIn.Application.ActiveDocument
        app = Globals.ThisAddIn.Application

        ' Durchlaufen aller Seiten im Dokument
        For pageNum As Integer = 1 To doc.ComputeStatistics(Word.WdStatistic.wdStatisticPages)
            app.Selection.GoTo(What:=Word.WdGoToItem.wdGoToPage, Which:=Word.WdGoToDirection.wdGoToAbsolute, Count:=pageNum)
            range = app.Selection.Range
            shapes = range.ShapeRange
            Dim ignorePage As Boolean = False

            ' Überprüfen jedes Shapes in der aktuellen Auswahl
            For Each shape As Word.Shape In shapes
                If shape.Title = "IgnorePageInPrint" Then
                    ignorePage = True
                    Exit For
                End If
            Next

            ' Wenn die Seite nicht ignoriert werden soll, füge sie zur Liste hinzu
            If Not ignorePage Then
                printablePages.Add(pageNum)
            End If
        Next
    ...
    Sub ExportSpecificPages()
        ...
        Dim tempDoc As Word.Document = app.Documents.Add()
        Dim printablePages As List(Of Integer) = GetPrintablePages()

        sourceDoc = Globals.ThisAddIn.Application.ActiveDocument
        app = Globals.ThisAddIn.Application

        If printablePages.Count > 0 Then
            For Each pageNum In printablePages
                sourceDoc.Activate()
                app.Selection.GoTo(What:=Word.WdGoToItem.wdGoToPage, Which:=Word.WdGoToDirection.wdGoToAbsolute, Count:=pageNum)
                app.Selection.Bookmarks("\Page").Range.Copy()

                ' Füge den Inhalt in das temporäre Dokument ein
                tempDoc.Activate()
                If tempDoc.Content.Text.Length > 0 Then
                    tempDoc.Content.InsertParagraphAfter() ' Fügt einen Absatz am Ende ein, um Seiten zu trennen
                End If
                tempDoc.Bookmarks("\EndOfDoc").Range.Paste()
            Next
        End If
    End Sub

    Private Sub btnPrintIgnorePages_Click(sender As Object, e As RibbonControlEventArgs) Handles btnPrintIgnorePages.Click
        ExportSpecificPages()
    End Sub
End Class
